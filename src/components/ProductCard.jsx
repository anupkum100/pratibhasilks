...
export default function ProductCard({ product }) {
  ...
  return (
    ...
    >
      ...
      <img 
        src={product.image} 
        className={`h-80 w-full object-cover ${product.sold ? 'grayscale' : ''}`} 
        alt={product.name}
      />
      <div className="p-4">
        ...
        <div className="mt-2">
          {product.offerPrice ? (
            <>
              ...
              <span className="font-bold" style={{ color: 'var(--color-secondary)' }}>₹ {product.offerPrice}</span>
            </>
          ) : (
            <p className="font-bold" style={{ color: 'var(--color-secondary)' }}>₹ {product.price}</p>
          )}
        </div>
        ...
        {product.sold ? (
          <button 
            disabled
            className="inline-block mt-4 px-4 py-2 rounded-lg bg-gray-400 text-white cursor-not-allowed"
          >
            Sold Out
          </button>
        ) : (
          <Link
            ...
            className="inline-block mt-4 text-white px-4 py-2 rounded-lg transition-colors hover:opacity-90"
